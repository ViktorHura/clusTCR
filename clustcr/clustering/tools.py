...
def create_edgelist(cdr3, filename=None, metric=None):
    '''
    ...
    if metric is None:
        ...
        for combo in combinations(list(cdr3), 2):
            f = max(len(combo[0]), len(combo[1]))
            d = aligner.score(combo[0], combo[1]) / f
            if d >= 5.125:
...
def create_edgelist_mod(cdr3, extended, model):
    ...
    pairs = tree.query_pairs(0.0035  , p=2)
